...
export type TaskScriptConfiguration<
  ...
  S,
  D = Array<
    `canisters:${keyof C}` | `scripts:${keyof S}` | "canisters:*" | "scripts:*"
  >,
> = {
  dependencies?: D
...
export type TaskConfiguration<T, D = Array<keyof T>> = {
  name: string
...
export const deployCanister = async (
  canisterName: string,
  ...
  try {
    ...
    if (canister_id) {
      try {
        const status = await mgmt.canister_status({
          canister_id: Principal.fromText(canister_id),
        })
        console.log(
          `Canister ${canisterName} (${canister_id}) exists. Status:`,
          status,
        )
        ...
        console.log(
          `Canister ${canisterName} exists but has no module installed. Proceeding with installation.`,
        )
      } catch (error) {
    ...
    let encodedArgs = canisterConfig.dfx_js?.args
    ...
    throw {
      kind: "CanisterDeploymentError",
      message: `Failed to deploy canister ${canisterName}: ${e.message}`,
    }
  }
}
...
export const deployCanisterEffect = (
  ...
  Effect.gen(function* (_) {
    const { agent, identity } = yield* DfxEnvironment
...
export const execTasks = async (
  taskStream: Repeater<any>,
  currentNetwork: string = "local",
) => {
  for await (const { taskName: fullName, taskConfig } of taskStream) {
    ...
    if (taskType === "canisters") {
      try {
        ...
        try {
          const mod = await import(`${appDirectory}/canister_ids.json`, {
            assert: { type: "json" },
          })
      ...
      try {
        const actors = await createActors([taskName], {
    ...
    if (taskType === "scripts") {
      const taskResult =
        taskConfig instanceof Promise ? await taskConfig : taskConfig
      console.log("Task finished:", fullName)
...
export const createTaskStream = (
  dfxConfig: DfxTs,
  tasks: Array<TaskFullName>,
) =>
  new Repeater<any>(async (push, stop) => {
    const jobs = tasks.map(async (fullName) => {
      ...
      const taskResults = Object.assign(
        {},
      ...
      if (taskType === "scripts") {
        if (isJustFn) {
          ...
          finalConfig =
            typeof taskConfig.fn === "function"
              ? taskConfig.fn({ ...taskResults })
              : taskConfig.fn
        }
      }
...
>(
  ...
  // Create a stream from the tasks array
  return Stream.fromIterable(tasks).pipe(
    Stream.mapEffect((fullName: TaskFullName) =>
      Effect.gen(function* () {
        ...
        if (dfxConfig instanceof Promise) {
          finalConfig = yield* Effect.promise(() => finalConfig)
        }

        return { taskName: fullName, taskConfig: finalConfig }
      }),
    ),
  )
}
...
export const transformWildcards = (
  ...
  const allTasks = isWildcard
    ? [
        ...new Set([
          ...(hasCanisterWildcard
            ? Object.keys(dfxConfig.canisters).map<TaskFullName>(
                (canisterName) => `canisters:${canisterName}`,
              )
            : []),
          // TODO: .scripts might be undefined
          ...(hasScriptWildcard
            ? Object.keys(dfxConfig.scripts).map<TaskFullName>(
...
export const getDeps = (
  ...
  tasks
    .flatMap((task) => transformWildcards(dfxConfig, task))
...
export const runTasks = async (config: DfxTs, tasks: Array<TaskFullName>) => {
  ...
  const allTasks = [
    ...new Set([
      ...allDeps,
      ...tasks.flatMap((t) => transformWildcards(config, t)),
    ]),
  ]
...
>(
  ...
  Effect.gen(function* () {
    ...
    const taskStream = createTaskStream(config, allTasks)
    yield* Effect.promise(() => execTasks(taskStream))
    const canisterIds = yield* getCanisterIdsEffect
    return canisterIds
  })

export const getDfxConfig = async (
  configPath: string = "crystal.config.ts",
) => {
  const appDirectory = fs.realpathSync(process.cwd())
  try {
    const { default: dfxConfig } = await import(
    ...
    throw {
      kind: "ConfigError",
      message: `Failed to get DFX config: ${e.message}`,
    }
  }
}
...
async function getEnv() {
  ...
  await agent.fetchRootKey().catch((err) => {
    ...
    console.error(err)
    throw {
      kind: "RootKeyFetchError",
      message: `Unable to fetch root key: ${err.message}`,
    }
  })
  return {
    network: "local",
...
export class DfxEnvironment extends Context.Tag("DfxEnvironment")<
  DfxEnvironment,
  {
...
export const DfxEnvironmentLive = Layer.effect(
  DfxEnvironment,
  Effect.gen(function* () {
...
export async function createActors(
  ...
  try {
    ...
    throw {
      kind: "ActorCreationError",
      message: `Failed to create actors: ${e.message}`,
    }
  }
}
...
export const createActorsEffect = <T>({
  ...
  Effect.gen(function* () {
    const { agent } = yield* DfxEnvironment
    const fs = yield* FileSystem.FileSystem
    const path = yield* Path.Path
    const appDir = yield* Config.string("APP_DIR")
...
export const getIdentity = async (
  ...
  try {
    ...
    const identityExists = fs.existsSync(
      `${os.homedir()}/.config/dfx/identity/${identityName}/identity.pem`,
    )
    if (!identityExists) {
      throw { kind: "IdentityDoesNotExist" }
    }
    let pem = fs.readFileSync(
      `${os.homedir()}/.config/dfx/identity/${identityName}/identity.pem`,
      "utf8",
    )
    pem = pem
    ...
    throw {
      kind: "IdentityError",
      message: `Failed to get identity: ${e.message}`,
    }
  }
}
...
//   const browser = await new Promise<{ accountId: string; principal: string }>(
...
export const getCanisterIds = (): CanisterIds => {
  try {
    const ids: CanisterIds = JSON.parse(
      fs.readFileSync(`${appDirectory}/canister_ids.json`, "utf8"),
    )
    ...
    throw {
      kind: "ConfigError",
      message: `Failed to get canister IDs: ${e.message}`,
    }
  }
}
