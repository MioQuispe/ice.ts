export const idlFactory = ({ IDL }) => {
  ...
  const Result_5 = IDL.Variant({ 'Ok' : IDL.Null, 'Err' : EgoError });
  ...
  const Result_6 = IDL.Variant({ 'Ok' : IDL.Vec(Payment), 'Err' : EgoError });
  return IDL.Service({
    ...
    'ledger_main_init' : IDL.Func([LedgerMainInitRequest], [Result_5], []),
    'ledger_payment_add' : IDL.Func([LedgerPaymentAddRequest], [Result_5], []),
    'ledger_payment_list' : IDL.Func([], [Result_6], []),
