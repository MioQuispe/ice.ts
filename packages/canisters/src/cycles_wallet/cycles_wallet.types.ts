...
export type WalletResult = { 'Ok' : null } |
  { 'Err' : string };
export type WalletResultCall = { 'Ok' : { 'return' : Uint8Array | number[] } } |
  { 'Err' : string };
export type WalletResultCallWithMaxCycles = {
    'Ok' : { 'return' : Uint8Array | number[], 'attached_cycles' : bigint }
  } |
  { 'Err' : string };
...
export interface _SERVICE {
  ...
  'wallet_call128' : ActorMethod<
    ...
    WalletResultCall
  >,
  'wallet_call_with_max_cycles' : ActorMethod<
    [
      {
        'args' : Uint8Array | number[],
        'method_name' : string,
