...
const createClient = (config: Config) => {
  ...
  const self = {
    async init() {
      ...
      try {
        ...
        for (const provider of _store.getValue().providers) {
          ...
          if (providerStatus === PROVIDER_STATUS.CONNECTED || providerStatus === PROVIDER_STATUS.LOCKED) {
            connectedProvider = provider
            connectedProviderStatus = providerStatus
            break
          }
        }
        if (connectedProviderStatus === PROVIDER_STATUS.LOCKED || connectedProviderStatus === PROVIDER_STATUS.CONNECTED) {
          _store.updateValue({
    ...
    async disconnect(args) {
      ...
      try {
        await self.activeProvider?.disconnect(args)
        _store.updateValue({
          status: CLIENT_STATUS.IDLE,
          activeProvider: undefined,
        })
    ...
    async createAnonymousActor(canisterId, interfaceFactory) {
      // TODO: add to query cache?
      // let anonymousActorQuery
      // anonymousActorQuery = createAnonymousActorQuery(canisterId, IDL.idlFactory)
      // const actor = await queryClient.fetchQuery(anonymousActorQuery.queryKey, anonymousActorQuery.queryFn)
